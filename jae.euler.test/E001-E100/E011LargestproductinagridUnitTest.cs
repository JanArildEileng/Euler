...
{
    public class E011LargestproductinagridUnitTest
    {
...
";
        ...
        private long[][] GetGrid()
        {
          var t= gridstring.Split(new char[] { ' ', '\n' });

            int row = 0;
            int col = 0;
            long[][] tabell = new long[20][];

            for (row = 0; row < 20; row++)
            {
                tabell[row] = new long[20];
                for (col = 0; col < 20; col++) {

                    tabell[row][col] = long.Parse(t[row * 20 + col + 1]);
                }
            }
